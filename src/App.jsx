...
function worldToCanvas(x, y, cx, cy, ppi) {
    return {cx: cx - y * ppi, cy: cy - x * ppi};
}

function canvasToWorld(cx, cy, cx0, cy0, ppi) {
    return {x: (cy0 - cy) / ppi, y: (cx0 - cx) / ppi};
}
...
function perpendicularHeading(dx, dy, left = true) {
    const vx = left ? -dy : dy, vy = left ? dx : -dx;
    return radToDeg(Math.atan2(vy, vx));
}

function headingVectorCanvas(h, len) {
    const vx = Math.cos(degToRad(h)), vy = Math.sin(degToRad(h));
    return {dx: -vy * len, dy: -vx * len};
}

function rotateLocalToWorld(lx, ly, h) {
    const a = degToRad(h);
...
export default function App() {
    ...
    useEffect(() => {
        stopPreview();
    }, [points, startPose.x, startPose.y, startPose.h]);
    ...
    useEffect(() => {
        ...
        if (livePose) {
            ...
            // Center dot for live robot pose
            octx.save();
            octx.fillStyle = "#ffffff";
            octx.strokeStyle = "#ff6ad5";
            octx.lineWidth = 2;
            octx.beginPath();
            octx.arc(pos.cx, pos.cy, 3, 0, Math.PI * 2);
            octx.fill();
            octx.stroke();
            octx.restore();
        }
    ...
    function drawPath(ctx) {
        ...
        ctx.lineWidth = 3;
        ctx.beginPath();
        let a = worldToCanvas(sx, sy, center.x, center.y, ppi);
    ...
    function drawStartMarker(ctx) {
        ...
        ctx.fill();
        const v = headingVectorCanvas(sh, 20);
        drawArrow(ctx, cx, cy, cx + v.dx, cy + v.dy, "#ffd166");
        ctx.restore();
    }
    ...
    function drawRectFootprint(ctx, x, y, h, L, W, options = {}) {
        ...
        const cornersWorld = cornersLocal.map(v => {
            const w = rotateLocalToWorld(v.x, v.y, h);
        ...
        for (let i = 1; i < c.length; i++) ctx.lineTo(c[i].cx, c[i].cy);
        ctx.closePath();
        ctx.globalAlpha = options.alpha ?? 0.16;
        ctx.fillStyle = options.fill ?? "#6be675";
        ctx.fill();
        ctx.globalAlpha = 1;
        ctx.lineWidth = 2;
        ctx.strokeStyle = options.stroke ?? "#6be675";
        ctx.stroke();
        ctx.restore();
    }
    ...
    function drawPointMarkersAndFootprints(ctx) {
        ...
        points.forEach((p, i) => {
            ...
            ctx.fill();
            const v = headingVectorCanvas(h, 18);
            drawArrow(ctx, cx, cy, cx + v.dx, cy + v.dy, "#6be675");
        });
        if (points.length > 0) {
    ...
    function drawPreview(ctx) {
        ...
        drawRectFootprint(ctx, preview.x, preview.y, preview.h, robotL, robotW, {
            fill: "#94e2b8",
            stroke: "#94e2b8",
            alpha: .10
        });
    ...
    function onCanvasClick(e) {
        ...
        if (placeStart) {
            setStartPose(sp => ({...sp, x: s.x, y: s.y}));
            setPlaceStart(false);
            return;
        }

        if (shapeType === "line") {
            placePointAt(s.x, s.y);
            return;
        }
    ...
    function sampleCircularArcThrough(A, M, B) {
        ...
        const thA = Math.atan2(y1 - uy, x1 - ux), thM = Math.atan2(y2 - uy, x2 - ux),
            thB = Math.atan2(y3 - uy, x3 - ux);
        ...
        const r = Math.hypot(x1 - ux, y1 - uy), arcLen = Math.abs(r * delta),
            n = clamp(Math.ceil(arcLen / 1.0), 8, 240);
        const out = [];
        for (let i = 1; i <= n; i++) {
...
public static double TOLERANCE_IN = ${toFixed(Number(tolerance) || 0, 2)};`;
    ...
    function removeTag(i) {
        setTags(prev => prev.filter((_, k) => k !== i));
    }
                ...
                <div className="scroll">
                    ...
                    <section className="section">
                        <div className="inline">
                            ...
                            <select className="input" style={{flex: 1}} value={canvasSize}
                    ...
                    <section className="section">
                        ...
                        <div className="grid three">
                            <button className={`btn ${shapeType === 'line' ? 'primary' : ''}`} onClick={() => {
                                setShapeType('line');
                                setBezierTemp(null);
                                setArcTemp(null);
                            }}>Line
                            </button>
                            <button className={`btn ${shapeType === 'bezier' ? 'primary' : ''}`} onClick={() => {
                                setShapeType('bezier');
                                setArcTemp(null);
                            }}>Bezier
                            </button>
                            <button className={`btn ${shapeType === 'arc' ? 'primary' : ''}`} onClick={() => {
                                setShapeType('arc');
                                setBezierTemp(null);
                            }}>Arc
                            </button>
                    ...
                    <section className="section">
                        ...
                        <div className="grid two">
                            <button className={`btn ${mode === 'straight' ? 'primary' : ''}`}
                                    onClick={() => setMode('straight')}>Straight + End Heading
                            </button>
                            <button className={`btn ${mode === 'tangent' ? 'primary' : ''}`}
                                    onClick={() => setMode('tangent')}>Tangent (forward)
                            </button>
                        </div>
                        <div className="grid two">
                            <button className={`btn ${mode === 'orth-left' ? 'primary' : ''}`}
                                    onClick={() => setMode('orth-left')}>Orthogonal (left)
                            </button>
                            <button className={`btn ${mode === 'orth-right' ? 'primary' : ''}`}
                                    onClick={() => setMode('orth-right')}>Orthogonal (right)
                            </button>
                        </div>
                        {mode === 'straight' && (
                            <div className="row">
                                ...
                                <input className="number" type="text" value={String(endHeadingInput)}
                                       onChange={e => {
                                           ...
                                           if (v === "" || v === "-") setEndHeadingInput(v); else {
                                               const n = parseFloat(v);
                                               if (!isNaN(n)) setEndHeadingInput(normDeg(n));
                                           }
                                       }}/>
                            </div>
                        )}
                    ...
                    <section className="section">
                        ...
                        <div className="grid two">
                            <div>
                                ...
                                <input className="number" type="number" min={1} max={120} step={1} value={velocity}
                                       onChange={(e) => setVelocity(parseFloat(e.target.value))}/>
                            </div>
                            <div>
                                <label>Preview speed (in/s)</label>
                                <input className="number" type="number" min={1} max={120} step={1} value={playSpeed}
                        ...
                        <div className="grid two">
                            <div>
                                ...
                                <input className="number" type="number" min={0} step={0.1} value={tolerance}
                                       onChange={(e) => setTolerance(e.target.value)}/>
                            </div>
                            <div>
                                <label>Snap (in) <span className="small">(placement only)</span></label>
                                <input className="number" type="number" min={0} step={0.1} value={snapInches}
                            ...
                            <div><label>X (in)</label><input className="number" type="text" value={String(startPose.x)}
                                                             onChange={e => {
                                                                 ...
                                                                 if (v === "" || v === "-") setStartPose({
                                                                     ...
                                                                     const n = parseFloat(v);
                                                                     if (!isNaN(n)) setStartPose({...startPose, x: n});
                                                                 }
                                                             }}/></div>
                            <div><label>Y (in)</label><input className="number" type="text" value={String(startPose.y)}
                                                             onChange={e => {
                                                                 ...
                                                                 if (v === "" || v === "-") setStartPose({
                                                                     ...
                                                                     const n = parseFloat(v);
                                                                     if (!isNaN(n)) setStartPose({...startPose, y: n});
                                                                 }
                                                             }}/></div>
                            <div><label>Heading (°)</label><input className="number" type="text"
                    ...
                    <section className="section">
                        <div className="grid two">
                            <div>
                                <h3>Grid Overlay</h3>
                                <div className="grid two">
                                    <select className="input" value={showGrid ? "on" : "off"}
                                            onChange={(e) => setShowGrid(e.target.value === "on")}>
                            ...
                            <div>
                                ...
                                <div className="grid two">
                                    <input className="number" type="number" value={robotL} min={1} max={36} step={0.5}
                                           onChange={(e) => setRobotL(parseFloat(e.target.value))}/>
                                    <input className="number" type="number" value={robotW} min={1} max={36} step={0.5}
                                           onChange={(e) => setRobotW(parseFloat(e.target.value))}/>
                                </div>
                    ...
                    <section className="section">
                        ...
                        <div className="inline">
                            <button className="btn ok" onClick={addTag}
                                    disabled={points.length === 0 || !tagName.trim()}>
                                Add Tag
                            </button>
                ...
                <div className="scroll">
                    ...
                    <section className="section">
                        ...
                        <div className="stats">
                            ...
                            <div className="stat">
                                <div className="k">Length (in)</div>
                                <div className="v">{toFixed(totalLen, 1)}</div>
                            </div>
                            <div className="stat">
                                <div className="k">Est. Time (s)</div>
                                <div className="v">{toFixed((totalLen || 0) / Math.max(velocity || 1, 1), 1)}</div>
                            </div>
                        </div>
                        <div className="small">Progress: {totalLen > 0 ? Math.round((playDist / totalLen) * 100) : 0}%
                            • {toFixed(playDist, 1)} / {toFixed(totalLen, 1)} in
                        </div>
                    </section>
